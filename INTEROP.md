...
How **ledgermind** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `ledgermind` lives in the **Host / artifact forensics** cluster.
...
| from | relation | to |
|---|---|---|
| `ledgermind` | JSON findings compose with | [`memtriage`](https://github.com/cognis-digital/memtriage), [`evtxhunt`](https://github.com/cognis-digital/evtxhunt), [`browserforensics`](https://github.com/cognis-digital/browserforensics) |
| `ledgermind` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `ledgermind` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `ledgermind` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the host / artifact forensics cluster.** `ledgermind` output feeds the next tool:
```bash
ledgermind ... --format json > out.json          # this tool's findings (see `ledgermind --help`)
...
ledgermind ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
ledgermind ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
ledgermind ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `ledgermind`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `ledgermind` slots
