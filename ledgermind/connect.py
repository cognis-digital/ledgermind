"""Native cognis-connect emit for ledgermind — forward findings to any platform.

Maps ledgermind's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    ledgermind ... --format json | ledgermind-emit --to stix
    ledgermind-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('event', '').split(' ')[0] if rec.get('event') else ''
